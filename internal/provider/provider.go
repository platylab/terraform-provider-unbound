	...
	resp.Schema = schema.Schema{
		Attributes: map[string]schema.Attribute{
			...
			"username": schema.StringAttribute{
				Description: "Username of the user used to login to Unbound API. May also be provided via UNBOUND_USERNAME environment variable.",
				Required:    true,
			},
			"password": schema.StringAttribute{
				Description: "Password of the user used to login to Unbound API. May also be provided via UNBOUND_PASSWORD environment variable.",
				Required:    true,
			},
		},
	}
...
func (p *unboundProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	...
	if config.Username.IsUnknown() {
		resp.Diagnostics.AddAttributeError(
			path.Root("username"),
			"Unknown unbound Username",
			"The provider cannot create the unbound API client as there is an unknown configuration value for the unbound API username. "+
	...
	if config.Password.IsUnknown() {
		resp.Diagnostics.AddAttributeError(
			path.Root("password"),
			"Unknown unbound Password",
			"The provider cannot create the unbound API client as there is an unknown configuration value for the unbound API Password. "+
				"Either target apply the source of the value first, set the value statically in the configuration, or use the UNBOUND_PASSWORD environment variable.",
		)
	}
	...
	password := os.Getenv("UNBOUND_PASSWORD")
	...
	if username == "" {
		resp.Diagnostics.AddAttributeError(
			path.Root("username"),
			"Missing unbound API Username",
			"The provider cannot create the unbound API client as there is a missing or empty value for the unbound API username. "+
	...
	if password == "" {
		resp.Diagnostics.AddAttributeError(
			path.Root("password"),
			"Missing unbound API Password",
			"The provider cannot create the unbound API client as there is a missing or empty value for the unbound API password. "+
				"Set the password value in the configuration or use the UNBOUND_PASSWORD environment variable. "+
				"If either is already set, ensure the value is not empty.",
		)
	}
	...
	ctx = tflog.SetField(ctx, "unbound_password", password)

	tflog.Debug(ctx, "Creating Unbound API client")

	// Create a new API client using the configuration values
	client, err := unboundapiclient.NewAuthenticatedClient(
		host,
		username,
		password,
	)
